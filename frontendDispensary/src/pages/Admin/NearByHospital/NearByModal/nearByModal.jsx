...
const NearByModal = (props) => {
  ...
  useEffect(() => {
    ...
    const handleSubmit= async(e)=>{
        ...
        if (inputField.name.trim().length === 0 || inputField.address.trim().length === 0 || inputField.address.trim().length === 0) return toast.error("Enter all the fields");
  ...
  return (
    <form className='register-form' onSubmit={handleSubmit}>
                <div className="register-form-div">
                    ...
                    <div className="register-input-box">
                        <input value={inputField.contact} onChange={(event)=>handleOnChange(event,"contact")} className='input-box-register' placeholder='Phone Number' type='text'/>
                    </div>
                </div>
